...
2. Run `npm run research` once to freeze one shared cutoff and retain the candidate run. Treat its local SearXNG results as a candidate inventory only. You must use the web search and web extraction tools directly to locate and read complete original publisher pages for selected stories. If `web_extract` has no backend, use the browser tool to open the public article; if the browser tool cannot start, use the terminal fallback `npx tsx scripts/extract-article.ts --url ARTICLE_URL --output research/runs/RUN_ID/source-N.json`, which uses the project's installed Playwright runtime to render and extract the page. If a page is blocked, follow the approved recovery ladder and pivot to an accessible original or reputable alternative. Continue with direct web retrieval rather than failing solely because one retrieval path is unavailable. Search snippets, RSS excerpts, aggregators, and AI-generated text are not evidence.
3. For every selected story, open and read the complete accessible article body, including meaningful qualifications, updates, corrections, lists, tables, and captions. Preserve the source's factual coverage, chronology, attribution, caveats and relevant context in the published reading version. When using `scripts/extract-article.ts`, require HTTP success, `complete: true`, at least three extracted paragraphs, and no warnings; retain its JSON evidence in the ignored run directory. Reject paywalled or incomplete pages, undated or future stories, duplicates, rumors, inaccessible sources, and stories outside the desk. Do not bypass access controls. Keep raw retrieval evidence only in ignored `research/runs/`.
4. Validate that each consequential story is real before selection: corroborate the material claims with at least one independent reputable source, and use two independent sources when the story is disputed, unusually consequential, or likely to be manipulated. Prefer an official primary record plus an independent newsroom or wire service. Record the corroborating URLs, matching facts, discrepancies, and why the evidence supports publication. If credible sources conflict or the story cannot be corroborated, mark it unverified and omit it rather than presenting it as fact.
...
6. Never pad a desk. If any desk has fewer than ten qualified stories, leave the current content, edition, approval record, and deployment untouched; write a dated failure report under `artifacts/`; report the per-desk shortfall; and stop without committing or pushing.
