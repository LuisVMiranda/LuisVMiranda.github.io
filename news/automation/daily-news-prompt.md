...
2. Run `npm run research` once to freeze one shared cutoff and retain the candidate run. Treat its local SearXNG results as a candidate inventory only. You must use the web search and web extraction tools directly to locate and read complete original publisher pages for selected stories. If the local full-text backend is unavailable or SearXNG returns noisy/undated results, continue with direct web retrieval from official sources and reputable journalism rather than failing solely because the local backend is unavailable. Search snippets, RSS excerpts, aggregators, and AI-generated text are not evidence.
